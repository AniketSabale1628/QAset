...
const Qa = ({ selectedType, quizName,handleCreateQuiz  }) => {
...
const handleContinueClickpop = () => {
  ...
  inputs.forEach((input, index) => {
    ...
    // Push the question text and options into the questions array
    const options = [];
    const radioButtons = document.querySelectorAll(`#text${index + 1} input[type="radio"]`);
    radioButtons.forEach(radioButton => {
      options.push({
        optionText: input.value.trim(), // Assuming each input corresponds to an option
        isCorrect: radioButton.checked // Check if this option is correct based on the radio button state
      });
    });
  ...
  if (allTextInputsFilled) {
    if (quizName.trim() === '' || selectedType === '') {
      alert('Please fill in quiz name and select a type before continuing.');
      return;
    }
  
    // Convert selectedTime to a valid number
    let timer;
    if (selectedTime === '5 sec') {
      timer = 5;
    } else if (selectedTime === '10 sec') {
      timer = 10;
    } else {
      // Default case when timer is 'OFF' or unknown value
      timer = 0;
    }
    let type=selectedType;
    alert(type);
    // If all text inputs are filled, construct the quizData object
    const quizData = {
      selectedType,
      quizName,
      questions,
      timer: selectedTime // Assuming selectedTime is defined elsewhere
    };
    console.log(quizData);
    // Call the handleCreateQuiz prop with the quizData
    // eslint-disable-next-line no-undef
    handleCreateQuiz(quizData);

    // Set continueClicked to true to indicate the continue button is clicked
    setContinueClicked(true);
    // Hide the create quiz section
    setShowCreateq(false);
  } else {
    // If any text input is not filled, show an alert to the user
    alert("Please fill in all question fields before continuing.");
  }
};
